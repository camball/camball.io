...
After pushing to main, I immediately got a build failure as the root directory was not configured for our new monorepo structure. In Vercel's general settings, I was able to [specify the proper root directory](https://vercel.com/docs/monorepos#using-monorepos-with-vercel-dashboard):

![Setting the root directory in Vercel's web UI](./assets/vercel_root_dir.png)
...
To deploy my blog subproject, `blog.mysite.io`, I created a new Vercel project. On setup, I specified my build command as per usual (`npm install`), but also specified my root directory, `apps/blog`. On project initialisation, the first build was a success. At this point, the only thing I had to do was add a DNS CNAME record for `blog` under my main `mysite.io` domain. Once done, both `mysite.io` and `blog.mysite.io` were fully in business, and I could hit both live.
